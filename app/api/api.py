...
api = Blueprint('api', __name__, url_prefix="/api")#To reveal the blueprint of backend

UPLOAD_FOLDER = os.path.abspath("app/static/attachments")#To enable upload of any folder in the attachments folder in static folder

@api.route('/login', methods=['POST'])
def login():# For login backend
    try:
...
def add_address():#To add a new address of the user
    try:
...
def create_order():#To make any order
    try:
        user_email = session.get("email")
        user_query = f"select * from users where email='{user_email}';"
        user = db.engine.execute(user_query).first()
...
def submit_help():#For the submit button in help section
...
def download(filename):#To download any file from UPLOAD_FOLDER
    return send_file(os.path.join(UPLOAD_FOLDER, filename), as_attachment=True)

@api.route("/search-order")
def search_order():#To search for the order
...
def execute():#To execute the code
    try:
...
def get_customer():#To get customer id
    try:
