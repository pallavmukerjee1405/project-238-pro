...
views = Blueprint('views', __name__, url_prefix="/")#To reveal the blueprint of frontend

@views.route('/')
@cross_origin()
def login():#For the login page which will direct to the dashboard
    try:
...
@cross_origin()
def dashboard():#For the dashboard or main window that comes up first after we login
    try:
...
@views.route('/profile')
@cross_origin()
def profile():#For the user profile in the profile option in the navbar
    try:
...
@views.route('/order')
@cross_origin()
def order():#For the order products window
    try:
...
@views.route("/help")
@cross_origin()
def help_page():#For the help page in the help option in the navbar
    try:
...
@views.route("/editor")
@cross_origin()
def editor():#For the editor page
    try:
