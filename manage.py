...
cli = FlaskGroup(create_app=create_app)#To start the application
#Below code containing dictionaries of all users personal details and products details
user_json = [
	{
		"name": "John Doe",
...
def recreate_db():#To reset the database
	db.drop_all()
	db.create_all()
	db.session.commit()

def seeder():#To seed the new data into the reset database
...
@cli.command()#To run the below function rsd separately in the terminal/cmd under manage.py as: pip install manage.py rsd
def rsd():
